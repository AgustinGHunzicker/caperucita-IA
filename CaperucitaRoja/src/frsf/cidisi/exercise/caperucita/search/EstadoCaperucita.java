...
public class EstadoCaperucita extends SearchBasedAgentState {
    ...
    public void updateState(Perception p) {
        ...
        for (int i = 0; i < ultimaPerception.getMovIzquierda(); i++)
            escenarioConocido[posY][posX - i] = EstadoCelda.VACIO;

        //Lugares libres para mover a la Derecha
        for (int i = 0; i < ultimaPerception.getMovDerecha(); i++)
            escenarioConocido[posY][posX + i] = EstadoCelda.VACIO;

        //Lugares libres para mover a Arriba
        for (int i = 0; i < ultimaPerception.getMovArriba(); i++) escenarioConocido[posY + i][posX] = EstadoCelda.VACIO;

        //Lugares libres para mover a Abajo
        for (int i = 0; i < ultimaPerception.getMovAbajo(); i++) escenarioConocido[posY - i][posX] = EstadoCelda.VACIO;
        ...
        if (!this.posicionLobo.equals(noExiste))
            escenarioConocido[(int) this.posicionLobo.getX()][(int) this.posicionLobo.getY()] = EstadoCelda.VACIO;
